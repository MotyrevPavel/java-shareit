...
public class Item {
    ...
    /**
     * Запрос на аренду, к которому привязан данный предмет.
     * Связь «многие‑к‑одному» с сущностью {@link ru.practicum.shareit.request.ItemRequest}.
     * Указывает, что предмет предложен в ответ на конкретный запрос пользователя о поиске предмета для аренды.
     *
